...
async function fetchLatest(){
    try {
        ...
        Object.assign(latestnews.value, res.data)
    } catch (error) {
        console.error(error)
        return []
    }
...
async function fetchPost(){
    try {
        ...
        const res = await axios.get(url)
        Object.assign(postnews.value, res.data)
    } catch (error) {
        console.error(error)
        return []
    }
...
<template v-for="(post,index) in postnews" :key="index">
          <div class="single_iteam" v-show="index==2"> 
            <a href="pages/si"> <img :src="post.img" alt=""></a>
...
<template v-for="(latest,index) in latestnews" :key="index">
              <li>
                <div class="media">     <RouterLink :to="`post-details?id=${latest.id}`" class="media-left"><img alt="" :src="latest.img"> </RouterLink> 
                  <div class="media-body"> <RouterLink :to="`post-details?id=${latest.id}`" class="catg_title">{{ latest.title }}</RouterLink> </div>
                </div>
              </li>
              </template>
